...
from .io import read_input_params, read_snaplist, read_global_xH
...
def electron_optical_depth(fname, volume_weighted=False):
    ...
    cosmo = cosmology.FlatLambdaCDM(
        H0=run_params["Hubble_h"] * 100.0, Om0=run_params["OmegaM"], Ob0=run_params["OmegaM"] * run_params["BaryonFrac"]
    )
    ...
    def cosmo_factor(z):
        return C.c * (1 + z) ** 2 / cosmo.H(z) * thomson_cross_section

    # read in the model run data
    snaps, z_list, _ = read_snaplist(fname)
    if not volume_weighted:
        xHII = 1.0 - read_global_xH(fname, snaps, weight="mass")
    else:
        xHII = 1.0 - read_global_xH(fname, snaps, weight="volume")
    ...
    with warnings.catch_warnings():
        ...
        sim_contrib = np.array(
            [integrate.simps(d_te_sim(z_list[: ii + 1], xHII[: ii + 1]), z_list[: ii + 1]) for ii in range(z_list.size)]
        )
